...
const GamifiedInstructionsPopup = () => {
  const t = useTranslations("SortingPage");

  // State
  const showInstructionsPopup = useSelector(
    (state: StateSchema) => state.sortingUi.showInstructionsPopup,
  );
  ...
  return (
    <Dialog
      ...
      fullWidth
    >
      <DialogTitle sx={{ pb: 2 }}>{t("instructions")}</DialogTitle>
      <IconButton
        aria-label="close"
        onClick={onClose}
        sx={(theme) => ({
      ...
      <DialogContent dividers sx={{ pt: 3, pb: 4 }}>
        ...
        <p>{t("gamified step1")}</p>

        <h4>{t("instructions step2")}</h4>
        <p>{t("gamified step2")}</p>

        <h4>{t("instructions step3")}</h4>
        <p>{t("gamified step3")}</p>
      </DialogContent>
    </Dialog>
  );
};

export default GamifiedInstructionsPopup;
