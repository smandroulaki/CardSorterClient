...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // State
  const NoParticipantsInStudy = useSelector(
    (state: StateSchema) => state.study.noParticipants,
  );
  const title = useSelector((state: StateSchema) => state.study.title);
  const description = useSelector(
    (state: StateSchema) => state.study.description,
  );
  const cards = useSelector((state: StateSchema) => state.study.cards);
  const isLive = useSelector((state: StateSchema) => state.study.isLive);
  const launchedDate = useSelector(
    (state: StateSchema) => state.study.launchedDate,
  );
  ...
  const downloadXLSX = () =>
    dispatch(studyAction.downloadXLSX({ studyId: id }));

  const onDuplicateStudy = () => {
    dispatch(
      createFromExistingStudy({
        title: title,
  ...
  const studyHeader = (
    <>
      <span className="header">
        ...
        <button
          className="share"
          onClick={() => setSharePopupShowing(true)}
          title="Share study"
        >
  ...
  return (
    <>
      <div className="study-page">
        ...
        {!pathname.includes("edit") && studyHeader}

        <Popup
          title={t("share url")}
          url={process.env.NEXT_PUBLIC_BASE_URL + `/sort/${id}`}
          secondtitle={t("share url (gamified)")}
          secondUrl={process.env.NEXT_PUBLIC_BASE_URL + `/sorter/${id}`}
        ...
        {NoParticipantsInStudy && !pathname.includes("edit") && (
          <div className="no-participants-page">
            ...
            <Image
              src="/card-sorter/images/empty.svg"
              alt="Nothing found"
              height={150}
              width={150}
            />
            ...
            <div className="actions-container">
              <InputWithCopy
                inputText={process.env.NEXT_PUBLIC_BASE_URL + `/sort/${id}`}
                title={t("share url")}
              />
            </div>
          </div>
        ...
        {pathname.includes("edit") && <>{children}</>}
      </div>
    </>
  );
}
