...
export default function Page() {
  ...
  const message = useSelector(
    (state: StateSchema) => state.studyCreation.thanksMessage,
  );
  const studyTitle = useSelector(
    (state: StateSchema) => state.studyCreation.title,
  );
  const studyDescription = useSelector(
    (state: StateSchema) => state.studyCreation.description,
  );
  const studyCards = useSelector(
    (state: StateSchema) => state.studyCreation.cards,
  );
  const studyMessage = useSelector(
    (state: StateSchema) => state.studyCreation.thanksMessage,
  );
  const link = useSelector(
    (state: StateSchema) => state.studyCreation.externalSurveyLink,
  );
  const studySendingStatus = useSelector(
    (state: StateSchema) => state.studyCreation.ui.studySendingStatus,
  );
  const sortType = useSelector(
    (state: StateSchema) => state.studyCreation.sortType,
  );
  const categories = useSelector(
    (state: StateSchema) => state.studyCreation.categories,
  );
  ...
  const onMessageChange = (e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    dispatch(
      studyCreationAction.changeThanksMessage({ message: e.target.value }),
    );
  };

  const onLink = (e: ChangeEvent<HTMLTextAreaElement>) => {
    dispatch(
      studyCreationAction.changeExternalSurveyLink({ link: e.target.value }),
    );
  };
  ...
  const onNext = () => {
    ...
    const sanitizedLink =
      link && link.trim() !== "" && link !== "undefined" ? link.trim() : "";

    dispatch(
      studyCreationAction.sendStudy({
        title: studyTitle || "",
  ...
  useEffect(() => {
    ...
    if (
      Object.values(studyCards).length === 0 &&
      studySendingStatus !== ActionStatus.SUCCESS &&
      ...
      <div className="bottom-container">
        <div className="btn-container">
          <Button
            ...
            variant="outlined"
            onClick={onPrev}
            disabled={studySendingStatus === ActionStatus.IS_FETCHING}
          >
          ...
          <Button
            ...
            variant="contained"
            onClick={onNext}
            loading={studySendingStatus === ActionStatus.IS_FETCHING}
          >
            {t("create")}
          </Button>
        </div>
